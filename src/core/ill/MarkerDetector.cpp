...
int MarkerDetector1::identifyMarker( const cv::Mat &in, int &nRotations ){
    ...
    emit showMarkerAndImage( vBWMarker, QList<bool>::fromVector( QVector<bool>::fromStdVector(vMarker) ));
 ...
 *
 ************************************/
MarkerDetector1::MarkerDetector1() :
    THRESHOLD(128)
{
    _doErosion=false;
 ...
 *
 ************************************/
void MarkerDetector1::setDesiredSpeed ( int val )
{
    if ( val<0 ) val=0;
    else if ( val>3 ) val=2;
 ...
 *
 ************************************/
void MarkerDetector1::detect ( const  cv::Mat &input,std::vector<Marker> &detectedMarkers, CameraParameters camParams ,float markerSizeMeters ,bool setYPerpendicular) throw ( cv::Exception )
{
 ...
 *
 ************************************/
void MarkerDetector1::detect ( const  cv::Mat &input,vector<Marker> &detectedMarkers,Mat camMatrix ,Mat distCoeff ,float markerSizeMeters ,bool setYPerpendicular) throw ( cv::Exception )
{
...
//     cv::cvtColor(grey,_ssImC ,CV_GRAY2BGR); //DELETE
    ...
    vector<vector<Marker> >markers_omp(1);
    vector<vector < std::vector<cv::Point2f> > >candidates_omp(1);
        ...
        if (resW) {
 		...
 		if(_cornerMethod==LINES) // make LINES refinement before lose contour points
		  refineCandidateLines( MarkerCanditates[i], camMatrix, distCoeff );
                markers_omp[0].push_back ( MarkerCanditates[i] );
                markers_omp[0].back().id=id;
                //sort the points so that they are always in the same order no matter the camera orientation
                std::rotate ( markers_omp[0].back().begin(),markers_omp[0].back().begin() +4-nRotations,markers_omp[0].back().end() );
            }
            else candidates_omp[0].push_back ( MarkerCanditates[i] );
        }
 ...
 *
 ************************************/
void  MarkerDetector1::detectRectangles ( const cv::Mat &thres,vector<std::vector<cv::Point2f> > &MarkerCanditates )
{
...
void MarkerDetector1::detectRectangles(const cv::Mat &thresImg,vector<MarkerCandidate> & OutMarkerCanditates)
{
    vector<MarkerCandidate>  MarkerCanditates;
...
//  	    reverse(MarkerCanditates[i].contour.begin(),MarkerCanditates[i].contour.end());//????
    ...
    vector< vector<pair<int,int>  > > TooNearCandidates_omp(1);
    #pragma omp parallel for
    for ( unsigned int i=0;i<MarkerCanditates.size();i++ )
    {
        ...
        for ( unsigned int j=i+1;j<MarkerCanditates.size();j++ )
        {
            ...
            if ( dist< 10 )
            {
                TooNearCandidates_omp[0].push_back ( pair<int,int> ( i,j ) );
            }
        }
    }
    //join
 ...
 *
 ************************************/
void MarkerDetector1::thresHold ( int method,const Mat &grey,Mat &out,double param1,double param2 ) throw ( cv::Exception )
{

    if (param1==-1) param1=_thresParam1;
    if (param2==-1) param2=_thresParam2;

    if ( grey.type() !=CV_8UC1 )     throw cv::Exception ( 9001,"grey.type()!=CV_8UC1","MarkerDetector1::thresHold",__FILE__,__LINE__ );
    switch ( method )
    {
    case FIXED_THRES:
 ...
 *
 ************************************/
bool MarkerDetector1::warp ( Mat &in,Mat &out,Size size, vector<Point2f> points ) throw ( cv::Exception )
{

    if ( points.size() !=4 )    throw cv::Exception ( 9001,"point.size()!=4","MarkerDetector1::warp",__FILE__,__LINE__ );
 ...
 *
 ************************************/
bool MarkerDetector1::warp_cylinder ( Mat &in,Mat &out,Size size, MarkerCandidate& mcand ) throw ( cv::Exception )
{

    if ( mcand.size() !=4 )    throw cv::Exception ( 9001,"point.size()!=4","MarkerDetector1::warp",__FILE__,__LINE__ );
 ...
 *
 ************************************/
bool MarkerDetector1::isInto ( Mat &contour,vector<Point2f> &b )
{
 ...
 *
 ************************************/
int MarkerDetector1:: perimeter ( vector<Point2f> &a )
{
    int sum=0;
 ...
 *
 */
void MarkerDetector1::findBestCornerInRegion_harris ( const cv::Mat  & grey,vector<cv::Point2f> &  Corners,int blockSize )
{
     SubPixelCorner Subp;
 ...
 *
 */
void MarkerDetector1::refineCandidateLines(MarkerDetector1::MarkerCandidate& candidate, const cv::Mat &camMatrix, const cv::Mat &distCoeff)
{
...
/**
 */
void MarkerDetector1::interpolate2Dline( const std::vector< Point2f >& inPoints, Point3f& outLine)
{
...
/**
 */
Point2f MarkerDetector1::getCrossPoint(const cv::Point3f& line1, const cv::Point3f& line2)
{
...
/**
 */
void MarkerDetector1::distortPoints(vector<cv::Point2f> in, vector<cv::Point2f> &out, const Mat& camMatrix, const Mat& distCoeff)
{
 	// trivial extrinsics
 ...
 *
 ************************************/
void MarkerDetector1::drawAllContours ( Mat input, std::vector<std::vector<cv::Point> > &contours )
{
 ...
 *
 ************************************/
void MarkerDetector1:: drawContour ( Mat &in,vector<Point>  &contour,Scalar color )
{
...
void  MarkerDetector1:: drawApproxCurve ( Mat &in,vector<Point>  &contour,Scalar color )
{
    for ( unsigned int i=0;i<contour.size();i++ )
    {
...
void MarkerDetector1::draw ( Mat out,const vector<Marker> &markers )
{
...
void MarkerDetector1::warpPerspective(const cv::Mat &in,cv::Mat & out, const cv::Mat & M,cv::Size size)
{
   //inverse the matrix
...
void MarkerDetector1::glGetProjectionMatrix ( CameraParameters &  CamMatrix,cv::Size orgImgSize, cv::Size size,double proj_matrix[16],double gnear,double gfar,bool invert ) throw ( cv::Exception )
{
    cerr<<"MarkerDetector1::glGetProjectionMatrix . This a deprecated function. Use CameraParameters::glGetProjectionMatrix instead. "<<__FILE__<<" "<<__LINE__<<endl;
...
void MarkerDetector1::setMinMaxSize(float min ,float max )throw(cv::Exception)
{
    if (min<=0 || min>1) throw cv::Exception(1," min parameter out of range","MarkerDetector1::setMinMaxSize",__FILE__,__LINE__);
    if (max<=0 || max>1) throw cv::Exception(1," max parameter out of range","MarkerDetector1::setMinMaxSize",__FILE__,__LINE__);
    if (min>max) throw cv::Exception(1," min>max","MarkerDetector1::setMinMaxSize",__FILE__,__LINE__);
    _minSize=min;
    _maxSize=max;
}
...
void MarkerDetector1::setWarpSize(int val) throw(cv::Exception)
{
  if (val<10) throw cv::Exception(1," invalid canonical image size","MarkerDetector1::setWarpSize",__FILE__,__LINE__);
  _markerWarpSize = val;
}
